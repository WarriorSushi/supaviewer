...
export default async function SubmitPage({ searchParams }: SubmitPageProps) {
  ...
  return (
    <main className="mx-auto w-full max-w-[96rem] px-4 pb-28 pt-6 sm:px-6 lg:px-10">
      <section className="grid gap-6 lg:grid-cols-[minmax(0,1.08fr)_minmax(22rem,0.92fr)]">
        <div className="sv-animate-in">
          ...
          <h1 className="mt-3 font-display text-5xl font-medium text-foreground sm:text-6xl">Submit a film</h1>
          ...
          {!profile ? (
            <div className="mt-8 rounded-xl border border-border/50 bg-card/60 px-4 py-4 text-sm text-muted-foreground">
              Sign in with a magic link before submitting. This keeps submissions attributable and reviewable.
            </div>
          ) : null}

          {success ? (
            <div className="mt-8 rounded-xl border border-[oklch(0.72_0.14_55_/_20%)] bg-[oklch(0.72_0.14_55_/_6%)] px-4 py-4 text-sm text-foreground">
              Submission received. It is now in the review queue and will be assigned a serial only if accepted.
            </div>
          ) : null}
          {error ? (
            <div className="mt-8 rounded-xl border border-[oklch(0.72_0.14_55_/_30%)] bg-[oklch(0.72_0.14_55_/_8%)] px-4 py-4 text-sm text-foreground">
              {error}
            </div>
          ) : null}
          ...
          <form
            action={submitFilm}
            className={`mt-8 grid gap-4 rounded-xl border border-border/50 bg-card p-6 ${!profile ? "pointer-events-none opacity-55" : ""}`}
          >
            <label className="block">
              <span className="sv-field-label">YouTube URL</span>
              <input
                className="sv-input"
                name="youtube_url"
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block">
                <span className="sv-field-label">Film title</span>
                <input
                  className="sv-input"
                  name="proposed_title"
              ...
              <label className="block">
                <span className="sv-field-label">Runtime</span>
                <input
                  className="sv-input"
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block">
                <span className="sv-field-label">Genre</span>
                <select className="sv-select" name="genre">
              ...
              <label className="block">
                <span className="sv-field-label">Format</span>
                <select className="sv-select" name="format">
            ...
            <label className="block">
              <span className="sv-field-label">Logline</span>
              <textarea
            ...
            <label className="block">
              <span className="sv-field-label">AI tools used</span>
              <input
                className="sv-input"
                name="tools"
        ...
        <aside className="grid gap-4 sv-animate-in sv-stagger-2">
